...
class Diffusion:
    ...
    def one_epoch(self, train=True, use_wandb=False): #TODO This is where I beleive code should be edited to incorporate AudioCLIPEmbeddings
        ...
        for i, (images, labels) in enumerate(pbar):  # Gets image and label from dataloader
            with torch.autocast("cuda") and (torch.inference_mode() if not train else torch.enable_grad()):
                images = images.to(self.device)
                labels = labels.to(self.device)  # Sends labels to cuda or cpu
    ...
    # This function prepares the dataloaders
